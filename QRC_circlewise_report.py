# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 15:42:41 2019
...
def create_df(data, group_no):
    ...
    print(df)
    top_area = flattened['SR_SUB_AREA1'][0]
    print(top_area)
    circlewise_table = data[data.SR_SUB_AREA1 == top_area].pivot_table(values ='CNT', index =['X_DIV_NAME'], 
                         columns =['CREATED_DATE'], aggfunc = np.sum)
...
#              'Circle wise count for ' + df[0]['SR_SUB_AREA1'][0], [3000, 6500],
...
def circlewise_main(start, end, grouping_no, output_path):
    ...
    create_slide(df1, 'Circle wise count for ' + df1[0]['SR_SUB_AREA1'][0],
                 'Circle wise count for ' + df1[0]['SR_SUB_AREA1'][0], [150, 200],
                 'circlewise_pre', [0.2,3,2.5], 'Postpaid: ' + df1[0]['SR_SUB_AREA1'][0] + ' QRC')
    return "Created Successfuly circlewise plots."