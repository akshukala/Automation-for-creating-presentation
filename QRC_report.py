# -*- coding: utf-8 -*-
"""
Created on Fri Nov 15 10:34:38 2019
...
#from init import QRC_PATH, PPT_PATH, IMG_PATH, DATE_1, DATE_2
from constants import variable

warnings.filterwarnings("ignore")

def create_df(data, grouping_no):
    ...
    sorted_table = table.sort_values(table.columns.tolist(), axis=0, ascending=False).head(6)
    flattened = pd.DataFrame(sorted_table.to_records())
    print(flattened.columns)
    
    grouping_no = 1+ grouping_no
    x = 1 + grouping_no
    flattened['before'] = flattened.iloc[:,1:x].sum(axis=1) # Check date range and adjust the index accordingly.
    flattened['after'] = flattened.iloc[:, x:(x+grouping_no)].sum(axis=1) # Check date range and adjust the index accordingly.
...
# df = create_df(data)
# create_slide(df, 'qrc_prepaid', 'QRC Analysis Prepaid', 'Top Call Drivers in Prepaid', 'Prepaid QRC', [12000, 8000], 'QRC Analysis of Prepaid based on CRM data')
...
def qrc_normal_main(start, end, grouping_no, file_path, output_path):
    ...
    if grouping_no > 0:
        variable.DATE_1 = date_s[2] + "-0" + str(middle_date)  + " " + month
        variable.DATE_2 = "0" + str(middle_date + 1) + "-" + date_e[2]  + " " + month
    else:
    ...
    df = create_df(data, grouping_no)
    create_slide(df, 'qrc_prepaid', 'QRC Analysis Prepaid', 'Top Call Drivers in Prepaid', 'Prepaid QRC', [12000, 8000], 'QRC Analysis of Prepaid based on CRM data')
    data1 = pd.read_excel(variable.QRC_PATH, sheet_name="Postpaid")


    df1 = create_df(data1, grouping_no)
    create_slide(df1, 'qrc_postpaid', 'QRC Analysis Postpaid', 'Top Call Drivers in Postpaid', 'Postpaid QRC', [700, 500], 'QRC Analysis of Postpaid based on CRM data')
    return "Success !!!"